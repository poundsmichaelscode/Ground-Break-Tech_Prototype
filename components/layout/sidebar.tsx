...
export function Sidebar() {
  ...
  return <aside className="hidden border-r bg-card/80 backdrop-blur-xl lg:fixed lg:inset-y-0 lg:flex lg:w-72 lg:flex-col">
    ...
    <div className="m-4 rounded-3xl border bg-premium-gradient p-4"><div className="flex items-center justify-between"><p className="text-sm font-bold">{user.company}</p><Badge tone={user.plan === "premium" ? "green" : "blue"}>{user.plan}</Badge></div><p className="mt-1 text-xs leading-5 text-muted-foreground">Signed in as {user.title}. Role-based access is active.</p><button onClick={logout} className="mt-4 flex w-full items-center gap-2 rounded-2xl px-3 py-2 text-sm font-semibold text-red-500 hover:bg-red-500/10"><LogOut className="h-4 w-4" /> Logout</button></div>
  </aside>;
}
